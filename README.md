...
既存のアプリケーション開発において、どの仕組みを置き換えるものかといえば  
ORM が挙げられます。`eevee` を導入することによって高速な ORM + α が利用できるようになると考えても良いと思います。
...
アプリケーション開発のための作業用ディレクトリを切ってください
...
実は eevee 実行のための準備はこれで終了です。  
インストールした eevee を実行します。
...
※ eevee で Go のソースコードを自動生成する際、上述のクラスファイルというものを参照します。  
これについては後で重点的に説明します。
...
その方法は `repository.Repository` という共通のインスタンスから行います。
...
`model` パッケージが提供する機能は非常に強力です。  
その機能はあとでじっくり解説することにし、  
...
2. 読み出しのみであってもキャッシュの作成など書き込み操作が入る場合もあり、トランザクションを前提としても良いケースもある(※ `repository.New()` には設定によって `sql.Tx` 以外のトランザクションインスタンスを渡すことができます )  

のようなものがあります。

ですが、それでも読み出しのみのAPIに関してはトランザクションを作りたくない、  
何も書き込みがないとしても `tx.Commit()` を呼び出すのは怖いといった考え方もあるかもしれません。

現在の eevee では上記のような考え方に対する解は持っていませんが、  
...
モデルインスタンスは JSON 文字列への高速なレンダリングをサポートしているため、  
...
`repository.Repository` を作成する流れはすでに解説する必要はないと思います。  
...
この方法は直感的ではありますが `map[string]interface{}` を手動で作成する場合などは安全に行うことができないというデメリットもあります。

もう一つが `Save(context.Context)` です。  
これはモデルがもつ強力な機能のうちのひとつで、  
...
`Save()` を呼んだ際にレコードの更新処理が走ることになります。
...
`eevee` を OSS 化する上でもっとも困難だったのは、いかに企業独自の開発方法を排除して一般化するかということでした。  

実は OSS として公開しているこの `eevee` は `v2` として開発したもので、  
社内で長らく使ってきた `v1` は、企業独自の仕組みを少なからず含んでいました。  
OSS として公開するにあたり、どのように企業独自の仕組みを排除していくか。  
`v2` では プラガブル な設計を採用することでそれを解決しています。  
