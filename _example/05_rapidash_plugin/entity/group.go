...
func (e *Group) Struct() *rapidash.Struct {
	return rapidash.NewStruct("groups").
		FieldUint64("id").
		FieldString("name")
}

func (e *Group) EncodeRapidash(enc rapidash.Encoder) error {
	if e.ID != 0 {
		enc.Uint64("id", e.ID)
	}
...
func (e *Groups) EncodeRapidash(enc rapidash.Encoder) error {
	for _, v := range *e {
...
func (e *Group) DecodeRapidash(dec rapidash.Decoder) error {
	e.ID = dec.Uint64("id")
...
func (e *Groups) DecodeRapidash(dec rapidash.Decoder) error {
	decLen := dec.Len()
	values := make(Groups, decLen)
	for i := 0; i < decLen; i++ {
		var v Group
